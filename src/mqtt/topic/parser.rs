...
pub(crate) fn parse_topic_filter(topic_filter: &str) -> Result<Vec<MatchToken>> {
    let mut pattern = vec![];
...
pub(crate) fn parse_topic_name(topic_name: &str) -> Result<Vec<Token>> {
    let mut pattern = vec![];
